...
def vincenty_distances(X, Y):
    centers = list(map(tuple, Y.values.tolist()))
    pts = list(map(tuple, X.values.tolist()))
    distances = []
    for pt in pts:
        v_dist = []
        for center in centers:
            vdist = vincenty(pt, center) 
            if vdist is not None:
                v_dist.append(vdist)
            else:
                v_dist.append(np.NaN)
        distances.append(v_dist)
    
    return np.array(distances)


def gather_vincenty(df, hub_data):
    """
    Performs KMeans clustering and assigns cluster labels to the DataFrame.
    ...
    try:
        cluster_centers = hub_data[['lat_rad', 'long_rad']]
        # TODO: option for vincenty distance
        distances = vincenty_distances(df[['lat_rad', 'long_rad']], 
                                        cluster_centers)
        # clustering based on min distance from cluster/hub
        cluster_assignments = np.argmin(distances, axis=1)
        # distance-based clusters
        df['vincenty'] = cluster_assignments
...
def gather_agglo(df, hub_data):
    ...
    try: 
        cluster_centers = hub_data[['lat_rad', 'long_rad']]
        data = df[['lat_rad', 'long_rad']]
        agglo = AgglomerativeClustering(n_clusters = len(hub_data), 
                        linkage = 'average')
        agglo.fit(data)
        agglo.predict(data)
        df['agglo'] = agglo.labels_
...
def gather_clusters(df : pd.DataFrame, 
    ...
    #get cluster labels based on min distance from cluster
    df, status_vincenty = gather_vincenty(df, hub_data)
    # get cluster labels from kmeans algo
    df, status_kmeans = gather_kmeans(df, hub_data)
    
    status = f"{status_dbscan}, {status_hd}, {status_vincenty}, {status_kmeans}"
    return df, status
...
def apply_hub_map(df, hub_data, cols=['dbscan', 'hd', 'vincenty', 'kmeans'], hub='kmeans'):#, 'kmeans'
    """
...
def calculate_appointment_distances(hub_data, appointments):
    ...
    distances = vincenty_distances(appointments[['lat_rad', 'long_rad']], 
                                    cluster_centers)
...
def or_tools_solver(data_list, appointments_list, mechanics_per_hub, index):
    ...
    ## Constraints
    # C1: Each worker is assigned to at most 1 task (since it is ongoing)
    for i in range(num_workers):
        solver.Add(solver.Sum([x[i, j] for j in range(num_tasks)]) <= 1)

    # C2: Each task is assigned to exactly one worker.
    for j in range(num_tasks):
        ...
        for i in range(num_workers):
            for j in range(num_tasks):
                ...
                if x[i, j].solution_value() > 0.75:
                    ...
                    solution_list.append({'appointment_id': appointments_list[j],
                                          'hub_solver': hub})
...
def main_algorithm(appointments, ddf, mechanics_per_hub, time_interval):
    ...
    for current_time in dts:#time_bookings_list
        ...
        # get appointments that start within the time interval of current time
        appointments_assign = appointments_to_assign(_appointments, current_time)
        # get appointment ids of appointments to assign
        appointments_id_list = appointments_assign.loc[appointments_assign['hub_solver'].isnull(),'appointment_id'].tolist()
        
        appointments_slice = ddf[appointments_id_list]
        ...
        solution_list = or_tools_solver(data_list, appointments_id_list,
...
def find_similarity_score(appointments):
    ...
    appointments['is_the_same'] = appointments.apply(lambda x: x['actual_hub'] == x['hub_solver'] if pd.notna(x['actual_hub']) else np.NaN, axis=1)
    #similarity = np.floor(appointments['is_the_same'].sum()/len(appointments)*100)
    similarity = appointments['is_the_same'].sum()*100.0/appointments.is_the_same.value_counts().sum()
...
def cluster_appointments(appointments_source : pd.DataFrame, 
                         config : dict, 
                         init_mechanics_dict : dict,
    ...
    Args:
    -----
        - appointments_source : pd.DataFrame
            Dataframe containing appointment data.
        - config : dict
            Loaded dictionary of configuration file in JSON format.
        - init_mechanics_dict : dict
            Initial mechanics count per hub.
    ...
    hub_data, hubs, appointments, hub_mechanics, df_hub_service = gather_variables(mechanics_count_redash,
                                                                                   appointments_source)
    ...
    appointments.loc[appointments['hub_solver'].isnull(),'hub_solver'] = appointments.loc[appointments['hub_solver'].isnull(),f'{cluster_method}_hub']
